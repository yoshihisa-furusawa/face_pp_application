...
class Analyze(object):
    ...
    def choise_function(self, encode_image, api_kind):
        if api_kind == "Detect":
            return self.api.detect(image_base64=encode_image, return_landmark=1, return_attributes=",".join(self.config["Attribute_Detect"]))
        elif api_kind == "Dence":
    ...
    def compere(self, path_list, target_file=None):
        ...
        for i, each_file in enumerate(path_list):
            ...
            if each_file != target_file:
                ...
                output_list.append(output)
                json_name, file_name = self.get_json_name("Compere", name=target_file.split("/")[-1].split(".")[0])
        self.save_output(json_name, file_name, output_list)

    def get_json_name(self, api_kind, name=None, name_concat=False):
        if name is None:
            if not name_concat:
                return os.path.join(*[self.abs_path, self.output_dir_name, api_kind]), self.output_json_name + ".json"
            else:
                return os.path.join(*[self.abs_path, self.output_dir_name, api_kind, self.output_json_name + ".json"])
        else:
            if not name_concat:
                return os.path.join(*[self.abs_path, self.output_dir_name, api_kind]),  name + ".json"
            else:
                return os.path.join(*[self.abs_path, self.output_dir_name, api_kind, name + ".json"])
    ...
    def __call__(self, target_file=None, path_to_image=None):
        ...
        if self.config["USE_API_Detect"]:
            ...
            Detect_api_json_to_csv(path_to_json=self.get_json_name("Detect", name_concat=True))()
        ...
        if self.config["USE_API_Dense_Facial_Landmarks"]:
            ...
            Dence_api_json_to_csv(path_to_json=self.get_json_name("Dence", name_concat=True))()
...
class Detect_api_json_to_csv(object):
    def __init__(self, path_to_json, output_path=None, output_dir_name="result_by_image"):
        ...
        if output_path is None:
            if path_to_json[0] == "/":
                ...
                add_path = ""
            self.output_file_path = os.path.join(*[add_path, *path_to_json.split("/")[:-1], output_dir_name])
        else:
    ...
    def detect_face_landmark(self, data, output_path_each_face, index_name=["x_cood", "y_cood"]):
        df = pd.DataFrame([], index=index_name)
        for landmark_name, value in data["landmark"].items():
            df[landmark_name] = [value["x"], value["y"]]
            
        output_file_name = os.path.join(*[output_path_each_face, "landmark.csv"])
        df.to_csv(output_file_name)
    ...
    def __call__(self, output_dir=None):
        ...
        for each_data in self.data:
            ...
            for face_number, each_face_data in enumerate(each_data["faces"]):
                ...
                self.detect_face_landmark(data=each_face_data,
                                          output_path_each_face=output_path_each_face)
