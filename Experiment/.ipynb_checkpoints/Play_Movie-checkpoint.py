...
class make_dataset(object):
    ...
    """
    def __init__(self, path_to_config, base_output_dir="./results/"):
        """
    ...
    def movie_to_images(self, image_output_path=None):
        ...
        if image_output_path is None:
            image_output_path = os.path.join(*[self.output_path, self.config["image_dir_name"]])
        ...
        while(cap.isOpened()):
            ...
            if ret:
                ...
                if int(counts % pass_frame_number) == 0:
                    ...
                    cv2.imwrite(os.path.join(*[image_output_path, image_name]), cv2.resize(frame, (width, height)))
            else:
                break
    ...
    def make_movie_gui(self):
        fourcc, cap, width, height, _, _ = self.get_movie_config()
        ...
        GUI(window_h=window_h, window_w=window_w, movie_h=height, movie_w=width, out_writer=out_writer)()
    ...
    def __call__(self, USE_GUI=True):
        self.set_config()
...
if __name__ == '__main__':
    dataset = make_dataset(path_to_config= "./config_camera.yaml", base_output_dir="../results/")
    dataset()
